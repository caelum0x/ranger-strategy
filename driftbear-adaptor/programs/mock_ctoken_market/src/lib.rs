...
pub mod mock_ctoken_market {
    ...
    pub fn withdraw_market(ctx: Context<DepositOrWithdraw>, ctoken_amount: u64) -> Result<()> {
        ...
        ctx.accounts.market.decrement_ctokens_minted(ctoken_amount)?;
...
pub struct InitializeMarketAndUser<'info> {
    ...
    /// CHECK: ATA validated by constraint
    #[account(
        mut,
...
impl Market {
    fn liquidity_to_ctoken(&self, liquidity_amount: u64) -> u64 {
        if self.liquidity_deposited == 0 || self.ctokens_minted == 0 {
